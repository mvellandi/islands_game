defmodule IslandsEngine.Game do
  ...
  # customize default GenServer terminate function,
  # to delete the game state ONLY if a timeout happens
  ...
  # send GenServer an async message to formally check/set game state,
  # and in the meantime default to a fresh state
  # this is to complete the GameSupervisor.start_game function (which is synchronous)
  def init(name) do
  ...
  def handle_call({:position_island, player, key, row, col}, _from, state_data) do
    ...
    with {:ok, rules} <- Rules.check(state_data.rules, {:position_islands, player}),
      ...
      {:error, :invalid_island_type} -> {:reply, {:error, :invalid_island_type}}
    end
  end
  ...
  def handle_info(:timeout, state_data) do
    # :stop calls GenServer.terminate, passing in {:shutdown, :timeout} as the first argument (reason).
    {:stop, {:shutdown, :timeout}, state_data}
  end

  # Async call during game initialization
  # Formally set the game state to either: new game, or existing game (if crashed)
  ...
  defp reply_success(state_data, reply) do
    # update the saved game data. player1.name refers to the 
